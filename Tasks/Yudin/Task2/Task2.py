...
choose_time = input('Please, choose time: \n enter "1" for choose current time or "2" to enter the time manually - ')
...
if choose_time == '1':
    ...
    enter_time = input('Please enter time in format hh:mm - ')
    if len(enter_time) != 5:
        ...
        quit()
    elif not enter_time[:2].isdigit() or not enter_time[3:].isdigit():
        print('Wrong. Please enter time in numbers in format hh:mm')
        quit()
    time_hour = int(enter_time.split(':')[0])
    time_minute = int(enter_time.split(':')[1])
    if time_hour > 24:
        print('Incorrect hours value')
        quit()
    elif time_hour == 24 and time_minute > 0:
...
if time_minute == 0:
    if time_hour == 0 or time_hour == 24:
        ...
        print(dict_hh[time_hour][0], 'ровно')

elif time_minute <= 20:
    if time_hour == 12:
        print(dict_mm[time_minute][0], dict_hh[time_hour - 11][1])
        quit()
    print(dict_mm[time_minute][0], dict_hh[time_hour + 1][1])

elif 20 < time_minute < 45 and time_minute != 30:
    print(dict_mm[time_minute // 10 * 10][1], dict_mm[time_minute % 10][0], dict_hh[time_hour + 1][1])

elif time_minute == 30:
    print('половина', dict_hh[time_hour + 1][1])

elif time_minute >= 45:
    print('без', dict_mm[60 - time_minute][1], dict_hh[time_hour + 1][0])
